...
const stats = computed(() => {
  ...
  const soldProducts = analyticsList.value.sold_products || {
    amount: "0.00",
    quantity: 0,
  };
  ...
  <div class="mb-10">
    <div class="relative isolate overflow-hidden">
      ...
      <header class="pb-4 pt-6 sm:pb-6">
        <div
          class="flex w-full flex-wrap items-center gap-6 px-4 sm:flex-nowrap sm:px-6 lg:px-8"
        >
          ...
          >
            <a
              ...
              @click.prevent="setDateRange(item.action)"
              >{{ item.name }}</a
            >
          </div>
        </div>
      ...
      >
        <dl
          class="grid w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 lg:px-2 xl:px-0"
        >
          <!-- Total Orders -->
          <div
