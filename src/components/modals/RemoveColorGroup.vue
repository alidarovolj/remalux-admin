...
const removeColorGroup = async () => {
  loading.value = true

  try {
    await colors.removeColorGroup(modals.modal.modalData.id)
    if(colors.removedColorGroup) {
      notifications.showNotification("success", "Успешно", "Цветовая группа удалена");
    ...
    <div>
      ...
      <div class="mt-3 text-center sm:mt-5">
        <p class="text-base font-semibold leading-6 text-gray-900">
          Хотите удалить цветовую группу?
        </p>
        <div class="mt-2">
          <p class="text-sm text-gray-500">
            Вы уверены, что хотите удалить цветовую группу? Это действие нельзя отменить.
          </p>
        </div>
      </div>
    </div>
