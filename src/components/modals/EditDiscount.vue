...
const editDiscount = async () => {
  ...
  try {
    await discounts.editDiscount(modals.modal.modalData.id, form.value);
    if (discounts.editedDiscount) {
...
onMounted(async () => {
  ...
  form.value.user_id = modals.modal.modalData.user.id;
  form.value.product_variant_id = modals.modal.modalData.product_variant.id;
  form.value.start_date = formatDate(modals.modal.modalData.start_date);
  form.value.end_date = formatDate(modals.modal.modalData.end_date);
  form.value.value = modals.modal.modalData.value;
  form.value.title = modals.modal.modalData.title;
  form.value.note = modals.modal.modalData.note;
});
      ...
      <div>
        <div>
          ...
          <p class="text-sm mb-3">
            Заполните все поля для редактирования скидки для пользователя
          </p>
        </div>
        <div
